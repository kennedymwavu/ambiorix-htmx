...
#'
#' @param href Character vector. eg. c("/", "/about" "/contact")
#' @param label Character vector of same length as `href`.
#' Labels for the navigation links. eg. c("Home", "About", "Contact").
#' @param active String. The active/current page. One of the options from
...
create_nav <- \(
  href = c("/", "/movies"),
  label = c("Modal types", "Movies"),
  active = label[1]
) {
  nav_items <- lapply(seq_along(href), \(i) {
    class <- paste(
      "nav-link",
      if (label[i] == active) "active border-bottom border-dark"
    )
